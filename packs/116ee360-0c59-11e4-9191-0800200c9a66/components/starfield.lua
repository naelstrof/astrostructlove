...
local respawnStar = function( e, i, offscreen, xaxis, maxwidth )
    ...
    if not offscreen then
        e.stars[i]:setPos( Vector( w * love.math.random(), h * love.math.random() ) )
    else
        -- Place it off-screen in the way of our velocity
        if xaxis then
            e.stars[i]:setPos( Vector( math.abs( e.stars[i]:getPos().x - ( e.width + maxwidth ) ), e.height * love.math.random() ) )
        else
            e.stars[i]:setPos( Vector( e.width * love.math.random(), math.abs( e.stars[i]:getPos().y - ( e.height + maxwidth ) ) ) )
        end
    end
    local s = e.size + love.math.random() * e.sizeDeviation - love.math.random() * e.sizeDeviation
    e.stars[i]:setScale( Vector( s, s ) )
end
